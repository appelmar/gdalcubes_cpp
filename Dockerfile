...
RUN apt-get update && DEBIAN_FRONTEND="noninteractive" apt-get install -y software-properties-common libboost-all-dev cmake g++ libsqlite3-dev git supervisor wget

RUN DEBIAN_FRONTEND="noninteractive" apt-get install -y libnetcdf-dev libcurl4-openssl-dev libcpprest-dev doxygen graphviz
...
RUN DEBIAN_FRONTEND="noninteractive" apt-get install -y libxml2-dev libopenjp2-7-dev libhdf4-alt-dev libgdal-dev gdal-bin  # install libraries needed for some important drivers
