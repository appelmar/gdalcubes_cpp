...
#if (!file.exists("CHIRPS_MONTHLY.zip"))
#  download.file("https://uni-muenster.sciebo.de/s/G5KIlTO5TmNrLby/download", method=dl.method, destfile="CHIRPS_MONTHLY.zip",mode = "wb")

unzip("MOD11A2.zip", exdir = "MOD11A2")
unzip("MOD13A3.zip", exdir = "MOD13A3")
#unzip("CHIRPS_MONTHLY.zip", exdir = "CHIRPS_MONTHLY")
```
...
The code below combines all files with all bands and adds subdatasets identifiers to generate a chracter vector of GDAL dataset identifiers, which then can be put as first argument to `gcbs_create_image_collection()`. The second argument here referes to the image collection format and the third argument provides the name of the output image collection file (which is simply an SQLite database).
...
gcbs_create_image_collection(gdal_datasets, "MOD11A2", "MOD11A2.db")
```
...
In most cases, users want to specify the extent and resolution manually. Above, the temporal resolution the cube was 3 months whereas below, we define a custom data cube view with temporal resolution of one month.
...
MOD11A2.daynight_difference = gcbs_apply_pixel(MOD11A2.bandselect, "0.02*(LST_DAY-LST_NIGHT)",names = "LST difference")
...
gcbs_create_image_collection(gdal_datasets, "MOD13A3", "MOD13A3.db")
...
joined.cube = gcbs_join_bands(gcbs_select_bands(MOD13A3.cube, "NDVI"), gcbs_select_bands(MOD11A2.cube, c("LST_DAY", "LST_NIGHT")))
joined.cube
```

# Applying R functions over chunks

- complete time series in one chunk
...
plot(gcbs_chunk_apply(joined.cube, f), zlim=c(-1,1))
```
...
# Future work
This vignette presented a very simple example with a small dataset. Future vignettes will demonstrate how to process larger imagery as from Sentinel 2. 
