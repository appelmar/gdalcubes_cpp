...
std::shared_ptr<chunk_data> select_time_cube::read_chunk(chunkid_t id) {
    ...
    for (uint32_t it=0; it < size_tyx[0]; ++it) {
        datetime t = std::dynamic_pointer_cast<cube_stref_labeled_time>(_st_ref)->datetime_at_index(ccoords[0] * _chunk_size[0] + it);
        uint32_t iin = _in_cube->st_reference()->index_at_datetime(t);
        if (iin >= 0 && iin < _in_cube->size_t()) {
            // COPY values
            auto inccoords = ccoords;
            inccoords[0] = iin / _in_cube->chunk_size()[0];
            chunkid_t incid = _in_cube->chunk_id_from_coords(inccoords);
            if (!in_chunk) {
                in_chunk = _in_cube->read_chunk(incid);
                inid = incid;
            }
            else {
                if (inid != incid) {
